...
def predict_results(model, sep_len, sep_wid, pet_len, pet_wid):
    ...
    # Return the results with prob
    return preds, prob_d

# Creating the web app interface
st.title("Iris Prediction Preject")
st.subheader("by Priyanka Tanaji Mane...")

# Take inputes from user
sep_len = st.number_input("Sepal Length", min_value=0.00, step= 0.02)
sep_wid = st.number_input("Sepal Width", min_value=0.00, step=0.02)
pet_len = st.number_input("Petal Length", min_value=0.00, step=0.02)
pet_wid = st.number_input("Petal Width", min_value=0.00, step=0.02)
